<template>
  <div class="card p-8">
    <h1 class="text-xl font-bold text-center mb-6">{{ $t('auth.register') }}</h1>

    <!-- OAuth buttons -->
    <div class="space-y-3 mb-6">
      <button @click="loginWithOAuth('google')" class="btn-secondary w-full flex items-center justify-center gap-3">
        ...
        {{ $t('auth.registerWith', { provider: 'Google' }) }}
      </button>

      <button @click="loginWithOAuth('github')" class="btn-secondary w-full flex items-center justify-center gap-3">
        ...
        {{ $t('auth.registerWith', { provider: 'GitHub' }) }}
      </button>
    </div>
    ...
    <div class="relative my-6">
      ...
      <div class="relative flex justify-center text-xs">
        <span class="bg-white px-3 text-gray-400">{{ $t('auth.orContinueWith') }}</span>
      </div>
    </div>
    ...
    <form @submit.prevent="handleRegister" class="space-y-4">
      ...
      <div>
        ...
        <input v-model="email" type="email" required class="input-field" placeholder="nom@exemple.com" />
      </div>
      <div>
    ...
    <p class="text-center text-sm text-gray-500 mt-6">
      {{ $t('auth.hasAccount') }}
      <NuxtLink to="/login" class="text-black font-medium hover:underline">{{ $t('auth.login') }}</NuxtLink>
    </p>
  </div>
</template>
